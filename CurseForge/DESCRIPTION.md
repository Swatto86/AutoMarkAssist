...
- Pull-wide Smart Group CC matching now gives limited CC options to the mobs that actually need them first, while using the shipped Smart Group CC icon map: Moon for Polymorph, Diamond for Sap, Square for Trap, Star for Shackle, Circle for Hibernate, and Triangle for Banish.
- The general mark-plan announce and preview output also honors the live party or raid roster, only showing CC labels for classes actually present and turning the remaining active icons into later kill-order lines.
...
- Manual announce and preview output for communicating the current kill and crowd-control plan without a separate legend editor.
