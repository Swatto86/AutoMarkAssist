...
### Database
- **Full TBC 5-Man Dungeon Database Rebuild:** Every Burning Crusade 5-man dungeon roster was re-verified against the Wowhead TBC Classic database, 2.4.3 creature data (creature types and CC-immunity masks), Wowpedia, and Icy Veins dungeon guides, replacing a list that contained invented mobs (e.g. "Hellfire Watchtower"), mobs assigned to the wrong dungeon (Hellfire Channeler in Ramparts, Bleeding Hollow Torturer in Blood Furnace), wrong creature types, and large gaps. All 16 dungeons now carry complete trash rosters — 249 entries — tuned for a warrior tank and party:
  - **Healers, summoners and reinforcement-callers are flagged Critical (kill-first):** e.g. Hellfire Watcher, Shadowmoon Summoner, Coilfang Scale-Healer, Murkblood Healer, Ethereal Priest/Spellbinder, Auchenai Necromancer, Cabal Acolyte/Summoner, Bloodwarder Mender/Physician, Negaton Warp-Master, Durnholde Warden, Sunblade Blood Knight/Physician.
  - **AoE / fear / mind-control threats are flagged High danger:** e.g. Coilfang Siren, Sethekk Prophet, Cabal Spellbinder, Nexus Terror, Fen Ray, Eredar Deathbringer, Ethereum Smuggler.
  - **CC-immune mobs are now correctly flagged** so CC marks are never wasted on them: Bonechewer Beastmaster; Shattered Hand Legionnaire, Gladiator, Champion and Centurion; Coilfang Champion and Defender; Bog Giant, Underbog Lord and Bog Overlord; Phantasmal Possessor (even Shackle-immune); Auchenai Monk; Fel Overseer and Malicious Instructor; Nexus Terror (Banish-immune despite being a Demon); Gargantuan Abyssal; the Tempest-Forge constructs; Mutate Fear-Shrieker/Horror; Rift Keeper and Rift Lord; Sunblade Sentinel.
  - **Creature types corrected for proper CC assignment:** the Sethekk Time-Lost casters are Undead (Shackle works), the Mechanar robots (Crusher/Driller/Wrecker) are Demons (Banish works), Underbog Lurkers/Shamblers are Elementals (Banish works), and the naga, arakkoa, ethereal and blood-elf rosters carry their correct Humanoid CC preferences.
  - **Trivial swarm adds, event-only spawns and de-aggroing slaves are marked SKIP** (e.g. Fel Orc Neophyte gauntlet waves, Wastewalker/Dreghood slaves, Mana Leeches, Reanimated Bones, Mechanar Tinkerers, Infinite Whelps, Sunblade Imps) so kill and CC marks stay on targets that matter.
  - Heroic-only CC immunities (e.g. Sethekk Ravenguard, Cobalt Serpent, Sunseeker Geomancer) are documented in the database comments and are picked up automatically in-game by the existing CC-immunity learner.
